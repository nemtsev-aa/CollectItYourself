...
public class GeneralSwitchingResult : ScriptableObject {
    public DateTime CurrentDate { get { return _currentDate; } private set { _currentDate = value; } }
    ...
    public float BuildingTime { get { return _switchingTimesValue; } private set { _switchingTimesValue = value; } }
    ...
    // Конструктор класса
    public GeneralSwitchingResult(DateTime currentDate, TaskData taskData, bool checkStatus, int switchBoxNumber, List<SingleSwitchingResult> singleSwichingResults,
    ...
    public static GeneralSwitchingResult CreateInstance(DateTime currentDate, TaskData taskData, bool checkStatus, int switchBoxNumber, 
        ...
        instance.BuildingTime = switchingTimesValue;
    ...
    public float GetSwitchingTimesValue() {
        if (_singleSwichingResults.Count() > 0) {
            ...
            foreach (SingleSwitchingResult iResult in _singleSwichingResults) {
                if (iResult != null) timesValue += iResult.BuldingTime;
            }
            return timesValue;
        } else {
