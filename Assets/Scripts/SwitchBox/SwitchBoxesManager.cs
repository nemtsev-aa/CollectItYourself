...
public class SwitchBoxesManager : MonoBehaviour, IService, CustomEventBus.IDisposable {
    ...
    public void Reset() {
        if (_switchBoxes.Count > 0) {
            foreach (var iSB in _switchBoxes) {
                Destroy(iSB.gameObject);
            }
            _switchBoxes.Clear();

            _switchBoxsSelectorView = null;
    ...
    public void Dispose() {
        _eventBus.Unsubscribe((TrainingModeStopSignal signal) => HideSwitchBoxs());
        _eventBus.Unsubscribe((TrainingModeStartSignal signal) => ShowSwitchBoxs());
    }
}
